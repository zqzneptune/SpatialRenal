#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8717007_processed.h5.

For this single sample
(GSM8717007, normal kidney, 92 weeks, female / 92 weeks, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8717007_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8717007_processed.qc.json``.

Provenance:
  gsm        : GSM8717007
  study      : kidney-aging-xenium (GSE286051)
  condition  : normal kidney, 92 weeks, female / 92 weeks
  species    : Mus musculus
  modality   : spatial (10x Xenium in situ)
  raw files  : GSM8717007_20210908-NMK92F-Fp1U1.barcodes.tsv.gz; GSM8717007_20210908-NMK92F-Fp1U1.cells.csv.gz; GSM8717007_20210908-NMK92F-Fp1U1.features.tsv.gz; GSM8717007_20210908-NMK92F-Fp1U1.matrix.mtx.gz; GSM8717007_20210908-NMK92F-Fp1U1.morphology.ome.tif.gz; GSM8717007_20210908-NMK92F-Fp1U1.transcripts.parquet.gz; GSM8717007_HE_image_20210908-NMK92F-Fp1U1.ome.tif.gz
...
SAMPLE_META: Dict[str, str] = {
    'gsm': 'GSM8717007',
    'sample_id': '20210908-NMK92F',
    'species': 'Mus musculus',
    'gse': 'GSE286051',
    'study': 'kidney-aging-xenium',
    'modality': 'spatial',
    'platform': '10x Xenium in situ',
    'disease_condition': 'normal (aging)',
    ...
    'panel_probes': 'Xenium 479 genes',
    'prep': 'FFPE',
    'reference_genome': 'mm10',
    'condition_detail': 'normal kidney, 92 weeks, female',
    'timepoint': '92 weeks',
}
...
# --------------------------------------------------------------------------- #
# Xenium staging + reading (GEO flat-file distribution)
# --------------------------------------------------------------------------- #
# Member suffix in the author-distributed files -> canonical 10x name that
# ``sc.read_10x_mtx`` accepts.
FLAT_MTX = {
    "barcodes": (".barcodes.tsv.gz", "barcodes.tsv.gz"),
    "features": (".features.tsv.gz", "features.tsv.gz"),
    "matrix": (".matrix.mtx.gz", "matrix.mtx.gz"),
}


def find_flat(raw_dir: Path, gsm: str, suffix: str) -> Optional[Path]:
    """Locate a flat per-GSM member by name suffix, e.g. ``<GSM>_<slide>.cells.csv.gz``."""
    hits = sorted(p for p in raw_dir.glob(f"{gsm}_*{suffix}") if p.is_file())
    return hits[0] if hits else None


def stage_mtx(raw_dir: Path, gsm: str, tmp_dir: Path) -> Path:
    """Stage the flat MTX trio under canonical 10x names (already gzipped -> copy)."""
    stage = tmp_dir / f"{gsm}_mtx"
    stage.mkdir(parents=True, exist_ok=True)
    for key, (src_suffix, canon) in FLAT_MTX.items():
        src = find_flat(raw_dir, gsm, src_suffix)
        if src is None:
            log("ERROR", f"missing {key} (*{src_suffix}) for {gsm} under {raw_dir}")
            sys.exit(3)
        shutil.copyfile(src, stage / canon)
    log("INFO", f"staged 10x MTX trio for {gsm} ({len(FLAT_MTX)} files)")
    return stage


def panel_summary(features_path: Path) -> Dict[str, Any]:
    """Summarise the panel from the flat features file (gene vs control targets)."""
    df = pd.read_csv(features_path, sep="\t", header=None,
                     names=["gene_id", "gene_name", "feature_type"])
    counts = {str(k): int(v) for k, v in df["feature_type"].value_counts().items()}
    return {
        "panel_name": None,
        "n_genes": counts.get("Gene Expression", 0),
        "n_features_total": int(len(df)),
        "feature_types": counts,
    }


def read_xenium_flat(matrix_dir: Path, cells_csv: Path, species: str) -> ad.AnnData:
    """Assemble a Xenium AnnData from the flat MTX trio + ``cells.csv.gz`` centroids."""
    ...
    adata.uns["xenium"] = panel_summary(matrix_dir / "features.tsv.gz")
...
def decompress_gz(src: Optional[Path], tmp_dir: Path) -> Optional[Path]:
    """Stream-decompress a gzipped image into tmp/ (``tifffile`` cannot read ``.gz``).

    The Xenium morphology image is a multi-GB BigTIFF; it is expanded once into
    the writable scratch mount and reused on subsequent runs.
    """
    if src is None or not src.exists():
        return None
    if not src.name.endswith(".gz"):
        return src
    dst = tmp_dir / src.name[:-3]
    if dst.exists() and dst.stat().st_size > 0:
        log("INFO", f"reusing decompressed image {dst.name} ({dst.stat().st_size / 1e9:.1f} GB)")
        return dst
    log("INFO", f"decompressing {src.name} -> {dst.name} (streaming, this takes a few minutes)")
    with gzip.open(src, "rb") as fi, open(dst, "wb") as fo:
        shutil.copyfileobj(fi, fo, 64 * 1024 * 1024)
    log("INFO", f"decompressed {dst.name} ({dst.stat().st_size / 1e9:.1f} GB)")
    return dst


def plot_umi_overlay(adata: ad.AnnData, gsm: str, out_png: Path, morph_tif: Optional[Path]) -> None:
    """UMI counts per cell overlaid on the fluorescence (morphology) image.
    ...
    if morph_tif is None or not morph_tif.exists():
        log("WARN", f"no morphology image for {gsm}; plotting centroids on plain background")
    ...
    xml = s.pages[0].tags.get(270)
    m = re.search(r'PhysicalSizeX="([\d.]+)"', xml.value) if xml is not None else None
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--raw-dir", default="raw", help="read-only original files (flat Xenium deposit)")
...
def main() -> None:
    args = parse_args()
    gsm = SAMPLE_META["gsm"]
    log("INFO", f"process start for {gsm} (Xenium flat files)")
    ...
    matrix_dir = stage_mtx(raw_dir, gsm, tmp_dir)
    cells_csv = find_flat(raw_dir, gsm, ".cells.csv.gz")
    if cells_csv is None:
        log("ERROR", f"missing cells.csv.gz for {gsm} under {raw_dir}")
        sys.exit(3)
    adata = read_xenium_flat(matrix_dir, cells_csv, SAMPLE_META.get("species", "NA"))
    log("INFO", f"read {adata.n_obs} cells x {adata.n_vars} genes for {gsm}")
    ...
    clean.uns["processing"] = {
        "script": "process.py (xenium_flat variant)",
    ...
    morph_tif = decompress_gz(find_flat(raw_dir, gsm, ".morphology.ome.tif.gz"), tmp_dir)
