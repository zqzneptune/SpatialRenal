#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8691123_processed.h5.

For this single sample
(GSM8691123, allogeneic, C57BL/6→BALB/c / —, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8691123_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8691123_processed.qc.json``.
...
Provenance:
  ...
  raw files  : GSM8691123_bl6.bc.2.morphology.ome.tif.gz; GSM8691123_bl6.bc.2.transcripts.zarr.zip
...
SAMPLE_META: Dict[str, str] = {
    'gsm': 'GSM8691123',
    'sample_id': 'B6→BALB/c allo 2 (Xenium)',
    'species': 'Mus musculus',
    'gse': 'GSE284742',
