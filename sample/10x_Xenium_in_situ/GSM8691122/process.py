#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8691122_processed.h5.

For this single sample
(GSM8691122, allogeneic, BALB/c→C57BL/6 / —, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8691122_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8691122_processed.qc.json``.

Provenance:
  gsm        : GSM8691122
  study      : kidney-transplant-rejection (GSE284742)
  condition  : allogeneic, BALB/c→C57BL/6 / —
  species    : Mus musculus
  modality   : spatial (10x Xenium in situ)
  raw files  : GSM8691122_bc.bl6.3.morphology.ome.tif.gz; GSM8691122_bc.bl6.3.transcripts.zarr.zip
...
SAMPLE_META: Dict[str, str] = {
    'gsm': 'GSM8691122',
    'sample_id': 'BALB/c→B6 allo 3 (Xenium)',
    'species': 'Mus musculus',
    'gse': 'GSE284742',
    ...
    'condition_detail': 'allogeneic, BALB/c→C57BL/6',
    'timepoint': '—',
}
