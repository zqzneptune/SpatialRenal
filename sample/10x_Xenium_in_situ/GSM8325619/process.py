#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8325619_processed.h5.

For this single sample
(GSM8325619, bIRI / 12 h L, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8325619_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8325619_processed.qc.json``.

Provenance:
  gsm        : GSM8325619
  study      : mouse-IRI-repair (GSE269884; Nat Commun s41467-025-62599-9)
  condition  : bIRI / 12 h L
  species    : Mus musculus
  modality   : spatial (10x Xenium in situ)
  raw files  : GSM8325619_xenium_hour12L_male_baysor_segmentation.tar.gz; GSM8325619_xenium_hour12L_male_output.tar.gz
