#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8691120_processed.h5.

For this single sample
(GSM8691120, syngeneic, BALB/c→BALB/c / —, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8691120_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8691120_processed.qc.json``.

Provenance:
  gsm        : GSM8691120
  study      : kidney-transplant-rejection (GSE284742)
  condition  : syngeneic, BALB/c→BALB/c / —
  species    : Mus musculus
  modality   : spatial (10x Xenium in situ)
  raw files  : GSM8691120_bc.bc.1.morphology.ome.tif.gz; GSM8691120_bc.bc.1.transcripts.zarr.zip
...
SAMPLE_META: Dict[str, str] = {
    'gsm': 'GSM8691120',
    'sample_id': 'BALB/c→BALB/c syn (Xenium)',
    'species': 'Mus musculus',
    'gse': 'GSE284742',
    ...
    'condition_detail': 'syngeneic, BALB/c→BALB/c',
    'timepoint': '—',
}
