#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8325620_processed.h5.

For this single sample
(GSM8325620, bIRI / 12 h R, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8325620_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8325620_processed.qc.json``.

Provenance:
  gsm        : GSM8325620
  study      : mouse-IRI-repair (GSE269884; Nat Commun s41467-025-62599-9)
  condition  : bIRI / 12 h R
  species    : Mus musculus
  modality   : spatial (10x Xenium in situ)
  raw files  : GSM8325620_xenium_hour12R_male_baysor_segmentation.tar.gz; GSM8325620_xenium_hour12R_male_output.tar.gz
