#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8691119_processed.h5.

For this single sample
(GSM8691119, syngeneic, C57BL/6→C57BL/6 / —, Xenium in situ spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8691119_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8691119_processed.qc.json``.

Provenance:
  gsm        : GSM8691119
  study      : kidney-transplant-rejection (GSE284742)
  condition  : syngeneic, C57BL/6→C57BL/6 / —
  species    : Mus musculus
  modality   : spatial (10x Xenium in situ)
  raw files  : GSM8691119_bl6.bl6.1.morphology.ome.tif.gz; GSM8691119_bl6.bl6.1.transcripts.zarr.zip
...
SAMPLE_META: Dict[str, str] = {
    'gsm': 'GSM8691119',
    'sample_id': 'B6→B6 syngeneic (Xenium)',
    'species': 'Mus musculus',
    'gse': 'GSE284742',
    ...
    'condition_detail': 'syngeneic, C57BL/6→C57BL/6',
    'timepoint': '—',
}
