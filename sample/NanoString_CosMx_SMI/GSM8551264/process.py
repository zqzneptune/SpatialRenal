#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8551264_processed.h5.

For this single sample
(GSM8551264, fetal, healthy / 19w0d, CosMx spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8551264_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8551264_processed.qc.json``.

Provenance:
  gsm        : GSM8551264
  study      : human-kidney-dev (GSE278614)
  condition  : fetal, healthy / 19w0d
  species    : Homo sapiens
  modality   : spatial (NanoString CosMx SMI)
  raw files  : GSM8551264_HK3524_raw.h5ad
...
SAMPLE_META: Dict[str, str] = {
    ...
    'species': 'Homo sapiens',
    'gse': 'GSE278614',
    'study': 'human-kidney-dev',
    'modality': 'spatial',
    'platform': 'NanoString CosMx SMI',
    'disease_condition': 'fetal (healthy)',
    'tissue_region': 'whole kidney',
    'sex': 'NA',
    'age': '19w0d',
    'treatment': 'NA',
    'model_genotype': 'NA',
    'patient_individual': 'HK3524',
    'panel_probes': 'CosMx 6k + 50 custom',
    'prep': 'FFPE',
    'reference_genome': 'hg38',
    'condition_detail': 'fetal, healthy',
    'timepoint': '19w0d',
}
...
# --------------------------------------------------------------------------- #
# Reading the provided CosMx h5ad
# --------------------------------------------------------------------------- #
def read_cosmx_h5ad(raw_dir: Path, gsm: str) -> ad.AnnData:
    candidates = sorted(raw_dir.glob(f"{gsm}*.h5ad")) or sorted(raw_dir.glob("*.h5ad"))
    if not candidates:
        log("ERROR", f"no h5ad found for {gsm} under {raw_dir}")
        sys.exit(3)
    adata = sc.read_h5ad(str(candidates[0]))
    log("INFO", f"read {candidates[0].name} ({adata.n_obs} x {adata.n_vars})")

    # CosMx stores coordinates in obs; promote to obsm["spatial"] if present
    if "CenterX_global_px" in adata.obs.columns and "CenterY_global_px" in adata.obs.columns:
        adata.obsm["spatial"] = adata.obs[["CenterX_global_px", "CenterY_global_px"]].values.astype(float)
        for c in ("CenterX_global_px", "CenterY_global_px"):
...
def run_qc(adata: ad.AnnData, species: str, thr: Dict[str, Any]) -> Tuple[Dict[str, Any], ad.AnnData]:
    ...
    metrics = {
        "gsm": None, "assay": "CosMx", "species": species,
...
def plot_umi_overlay(adata: ad.AnnData, gsm: str, out_png: Path) -> None:
    if "spatial" in adata.obsm:
        ...
        ax.set_title(f"{gsm} — UMI counts distribution (no coordinates)")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--raw-dir", default="raw", help="read-only original files (CosMx h5ad)")
...
def main() -> None:
    args = parse_args()
    gsm = SAMPLE_META["gsm"]
    log("INFO", f"process start for {gsm} (CosMx h5ad)")
    ...
    adata = read_cosmx_h5ad(raw_dir, gsm)
    ...
    clean.uns["processing"] = {
        "script": "process.py (cosmx_h5ad variant)",
