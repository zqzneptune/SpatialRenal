#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8551262_processed.h5.

For this single sample
(GSM8551262, fetal, healthy / 18w5d, CosMx spatial transcriptomics), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8551262_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8551262_processed.qc.json``.

Provenance:
  gsm        : GSM8551262
  study      : human-kidney-dev (GSE278614)
  condition  : fetal, healthy / 18w5d
  species    : Homo sapiens
  modality   : spatial (NanoString CosMx SMI)
  raw files  : GSM8551262_FK1_raw.h5ad
