#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM9938131_processed.h5.

For this single sample
(GSM9938131, kidney allograft; patient 5; normal control (non-transplant); peritubular capillary / —, GeoMx DSP spatial transcriptomics (region-based)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM9938131_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM9938131_processed.qc.json``.

Provenance:
  gsm        : GSM9938131
  study      : CAMR-GeoMx (GSE342778)
  condition  : kidney allograft; patient 5; normal control (non-transplant); peritubular capillary / —
  species    : Homo sapiens
  modality   : spatial (NanoString GeoMx DSP)
  raw files  : GSE342778_Hs_R_NGS_WTA_v1.0.pkc.gz; GSM9938131_DSP-1001660023291-D-G08.dcc.gz
...
SAMPLE_META: Dict[str, str] = {
    ...
    'tissue_region': 'peritubular capillary',
    'sex': 'NA',
    'age': 'NA',
    'treatment': 'NA',
    'model_genotype': 'NA',
    'patient_individual': 'patient 5',
    'panel_probes': 'GeoMx Human NGS WTA (18,677 targets)',
    'prep': 'FFPE',
    'reference_genome': 'hg38',
    'condition_detail': 'kidney allograft; patient 5; normal control (non-transplant); peritubular capillary',
    'timepoint': '—',
}
