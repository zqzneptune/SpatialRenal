#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM9938128_processed.h5.

For this single sample
(GSM9938128, kidney allograft; patient 5; normal control (non-transplant); glomerulus / —, GeoMx DSP spatial transcriptomics (region-based)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM9938128_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM9938128_processed.qc.json``.
...
Provenance:
  ...
  raw files  : GSE342778_Hs_R_NGS_WTA_v1.0.pkc.gz; GSM9938128_DSP-1001660023291-D-G05.dcc.gz
