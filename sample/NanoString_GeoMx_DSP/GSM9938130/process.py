#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM9938130_processed.h5.

For this single sample
(GSM9938130, kidney allograft; patient 5; normal control (non-transplant); glomerulus / —, GeoMx DSP spatial transcriptomics (region-based)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM9938130_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM9938130_processed.qc.json``.

Provenance:
  gsm        : GSM9938130
  study      : CAMR-GeoMx (GSE342778)
  condition  : kidney allograft; patient 5; normal control (non-transplant); glomerulus / —
  species    : Homo sapiens
  modality   : spatial (NanoString GeoMx DSP)
  raw files  : GSE342778_Hs_R_NGS_WTA_v1.0.pkc.gz; GSM9938130_DSP-1001660023291-D-G07.dcc.gz
...
SAMPLE_META: Dict[str, str] = {
    ...
    'condition_detail': 'kidney allograft; patient 5; normal control (non-transplant); glomerulus',
    'timepoint': '—',
}
