#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM9045323_processed.h5.

For this single sample
(GSM9045323, AKI repair; pooled WT+KO / —, Visium spatial transcriptomics (10x Space Ranger)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM9045323_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM9045323_processed.qc.json``.

Provenance:
  gsm        : GSM9045323
  study      : kidney-repair-matrix (GSE299736)
  condition  : AKI repair; pooled WT+KO / —
  species    : Mus musculus
  modality   : spatial (10x Visium)
  raw files  : GSM9045323_barcodes.tsv.gz; GSM9045323_features.tsv.gz; GSM9045323_matrix.mtx.gz; GSM9045323_tissue_hires_image.png.gz; GSM9045323_tissue_lowres_image.png.gz
...
SAMPLE_META: Dict[str, str] = {
    ...
    'species': 'Mus musculus',
    'gse': 'GSE299736',
    'study': 'kidney-repair-matrix',
    'modality': 'spatial',
    'platform': '10x Visium',
    'disease_condition': 'AKI repair',
    'tissue_region': 'whole kidney',
    'sex': 'NA',
    'age': 'NA',
    'treatment': 'NA',
    'model_genotype': 'WT and KO',
    'patient_individual': 'NA',
    'panel_probes': 'WTS (Visium)',
    'prep': 'NA',
    'reference_genome': 'mm10',
    'condition_detail': 'AKI repair; pooled WT+KO',
    'timepoint': '—',
}
