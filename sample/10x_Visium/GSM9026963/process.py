#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM9026963_processed.h5.

For this single sample
(GSM9026963, reference kidney; nephrectomy / —, Visium spatial transcriptomics (10x Space Ranger)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM9026963_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM9026963_processed.qc.json``.

Provenance:
  gsm        : GSM9026963
  study      : kidney-PT-regen (GSE298953)
  condition  : reference kidney; nephrectomy / —
  species    : Homo sapiens
  modality   : spatial (10x Visium)
  raw files  : GSM9026963_V10S15-102_XY01_IU-21-019-2.tif.gz; GSM9026963_aligned_fiducials.jpg.gz; GSM9026963_detected_tissue_image.jpg.gz; GSM9026963_filtered_feature_bc_matrix.h5; GSM9026963_scalefactors_json.json.gz; GSM9026963_tissue_hires_image.png.gz; GSM9026963_tissue_lowres_image.png.gz; GSM9026963_tissue_positions_list.csv.gz
...
SAMPLE_META: Dict[str, str] = {
    'gsm': 'GSM9026963',
    'sample_id': 'IU-21-019-2',
    'species': 'Homo sapiens',
    'gse': 'GSE298953',
    'study': 'kidney-PT-regen',
    'modality': 'spatial',
    'platform': '10x Visium',
    'disease_condition': 'reference (nephrectomy control)',
    'tissue_region': 'whole kidney',
    'sex': 'M',
    'age': 'NA',
    'treatment': 'NA',
    'model_genotype': 'NA',
    'patient_individual': 'IU-21-019-2',
    'panel_probes': 'WTS (Visium)',
    'prep': 'fresh frozen (OCT)',
    'reference_genome': 'hg38',
    'condition_detail': 'reference kidney; nephrectomy',
    'timepoint': '—',
}
...
# --------------------------------------------------------------------------- #
# File discovery + staging (Visium Space Ranger h5 family)
# --------------------------------------------------------------------------- #
def find_file(raw_dir: Path, keywords: list[str]) -> Optional[Path]:
    """First regular file in raw_dir whose name contains every keyword.

    Each sample's raw/ holds only that sample's files, so keyword matching over
    all files (no GSM-prefix assumption) safely handles both the
    ``GSMxxxxx_*`` and ``GSE<id>_GSMxxxxx_*`` naming conventions.
    """
    for f in sorted(raw_dir.iterdir()):
        if not f.is_file() or f.name.endswith("_raw_counts.h5"):
            continue
        name = f.name.lower()
...
def find_in_spatial(spatial_root: Path, keywords: list[str]) -> Optional[Path]:
    """First file anywhere under spatial_root whose name contains every keyword."""
    for f in sorted(spatial_root.rglob("*")):
        if not f.is_file():
            continue
...
def locate(spatial_root: Path, raw_dir: Path, keywords: list[str]) -> Optional[Path]:
    """Find a file by keyword in the staged spatial tree, else the top-level raw dir."""
    if spatial_root is not raw_dir:
        p = find_in_spatial(spatial_root, keywords)
        if p is not None:
            return p
    return find_file(raw_dir, keywords)


def stage_tar(raw_dir: Path, gsm: str, tmp_dir: Path) -> Tuple[Optional[Path], Path]:
    """Extract the 10x h5 + spatial essentials from a per-sample Space Ranger tar.

    Returns ``(h5_path, spatial_root)``. If no tar (or nothing relevant inside)
    is found, ``h5_path`` is None and ``spatial_root`` stays ``raw_dir``.
    """
    tars = [f for f in sorted(raw_dir.iterdir())
            if f.is_file() and f.name.lower().endswith((".tar", ".tar.gz"))]
    if not tars:
        return None, raw_dir
    stage = tmp_dir / f"{gsm}_visium_h5"
            ...
            with tarfile.open(tar, "r:*") as tf:
                for m in tf.getmembers():
                    if not m.isfile():
                        continue
                    low = m.name.lower()
                    if low.endswith("filtered_feature_bc_matrix.h5") or \
                       any(k in low for k in ("tissue_positions", "tissue_hires_image",
                                              "tissue_lowres_image", "scalefactors_json",
                                              "detected_tissue_image")):
                        tf.extract(m, path=stage, filter="data")
                        n += 1
        except (tarfile.TarError, OSError) as e:
            log("WARN", f"tar {tar.name} not readable: {e}")
    if n == 0:
        return None, raw_dir
    h5 = next(stage.rglob("filtered_feature_bc_matrix.h5"), None)
    log("INFO", f"staged Space Ranger outs from {tars[0].name} ({n} files)")
    return h5, stage
...
def load_positions(path: Path) -> pd.DataFrame:
    """Read tissue positions (csv with/without header, possibly gzipped)."""
...
def load_image(path: Path) -> Optional[np.ndarray]:
    try:
        return np.asarray(Image.open(io.BytesIO(read_bytes(path))).convert("RGB"))
...
def read_visium_h5(raw_dir: Path, gsm: str, tmp_dir: Path) -> ad.AnnData:
    """Assemble a Visium AnnData from a 10x ``filtered_feature_bc_matrix.h5``.

    Handles both author-distributed layouts:
      * extracted: the h5 + spatial files (possibly gzipped) at the top of ``raw/``;
      * tarred: a per-sample Space Ranger ``*_output.tar.gz`` with the ``outs/``
        subtree (``outs/filtered_feature_bc_matrix.h5`` + ``outs/spatial/``).
    """
    h5 = find_file(raw_dir, ["filtered_feature_bc_matrix", ".h5"])
    spatial_root = raw_dir
    if h5 is None:
        h5, spatial_root = stage_tar(raw_dir, gsm, tmp_dir)
    if h5 is None:
        log("ERROR", f"no filtered_feature_bc_matrix.h5 for {gsm} under {raw_dir}")
        sys.exit(3)
    log("INFO", f"reading 10x h5: {h5}")
    adata = sc.read_10x_h5(str(h5))
    adata.var_names_make_unique()
    library_id = gsm

    pos_path = locate(spatial_root, raw_dir, ["tissue_positions"])
    ...
    for key in ("hires", "lowres"):
        p = locate(spatial_root, raw_dir, [f"tissue_{key}_image"])
        if p is not None:
            img = load_image(p)
            if img is not None:
                images[key] = img
    if not images:
        p = locate(spatial_root, raw_dir, ["detected_tissue_image"])
        if p is not None:
            img = load_image(p)
            if img is not None:
                images["hires"] = img
    ...
    sf = locate(spatial_root, raw_dir, ["scalefactors"])
    if sf is not None:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--raw-dir", default="raw", help="read-only original files (Visium h5 / Space Ranger tar)")
...
def main() -> None:
    args = parse_args()
    gsm = SAMPLE_META["gsm"]
    log("INFO", f"process start for {gsm} (Visium-h5)")
    ...
    adata = read_visium_h5(raw_dir, gsm, tmp_dir)
    ...
    clean.uns["processing"] = {
        "script": "process.py (visium_h5 variant)",
