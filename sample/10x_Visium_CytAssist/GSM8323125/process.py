#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8323125_processed.h5.

For this single sample
(GSM8323125, bIRI / 6 wk, Visium spatial transcriptomics (10x Space Ranger)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8323125_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8323125_processed.qc.json``.
...
Provenance:
  ...
  raw files  : GSM8323125_visium_week6_male_spaceranger.tar.gz
