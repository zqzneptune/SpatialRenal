#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8323121_processed.h5.

For this single sample
(GSM8323121, bIRI / 4 h, Visium spatial transcriptomics (10x Space Ranger)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8323121_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8323121_processed.qc.json``.
...
Provenance:
  ...
  raw files  : GSM8323121_visium_hour4_male_spaceranger.tar.gz
