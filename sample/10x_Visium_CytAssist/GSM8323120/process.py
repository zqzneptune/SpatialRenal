#!/usr/bin/env python
"""Per-sample ingestion + QC: raw/ -> data/GSM8323120_processed.h5.

For this single sample
(GSM8323120, bIRI sham / sham, Visium spatial transcriptomics (10x Space Ranger)), parse the original author-distributed files in
``raw/`` into a standardized, QC-filtered AnnData stored under the unified name
``data/GSM8323120_processed.h5`` (h5ad-compatible HDF5), plus a matching QC report
``data/GSM8323120_processed.qc.json``.

Provenance:
  gsm        : GSM8323120
  study      : mouse-IRI-repair (GSE269884; Nat Commun s41467-025-62599-9)
  condition  : bIRI sham / sham
  species    : Mus musculus
  modality   : spatial (10x Visium CytAssist)
  raw files  : GSM8323120_visium_sham_male_spaceranger.tar.gz
